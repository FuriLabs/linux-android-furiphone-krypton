...
{
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor dummyline = %d, dummypixels = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
	//return; //for test
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
	...
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor framerate = %d, min framelength should enable %d \n", framerate,min_framelength_en);
...
static void write_shutter(kal_uint16 shutter)
{
	...
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    if (gain < BASEGAIN || gain > 32 * BASEGAIN) {
        pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor Error gain setting");
    ...
    spin_unlock(&imgsensor_drv_lock);
    pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
	...
	switch(image_mirror)
		{
		   ...
		   case IMAGE_HV_MIRROR:
			     ...
			     break;
        default:
        pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor Error image_mirror setting\n");
		}
}
...
{
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor func = %s \n",__func__);
...
{
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor E! \n");
    // full size 29.76fps
    // capture setting
// Pll Setting - VCO = 280Mhz
// write_cmos_sensor(0x0100, 0x0000);
...
{
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor E! currefps:%d\n",currefps);
    // full size 30fps
    // capture setting
// Pll Setting - VCO = 280Mhz
 // write_cmos_sensor(0x0100, 0x0000);
...
//prize add by lipengpeng 20200215 start
	...
	pr_debug("111LPP,gms3p3sxmodule_id=%x",gms3p3sxmodule_id);
	if(gms3p3sxmodule_id !=0xD5){
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001))+1;
			pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor read_0x0000=0x%x, 0x0001=0x%x,0x0000_0001=0x%x\n",read_cmos_sensor_8(0x0000),read_cmos_sensor_8(0x0001),read_cmos_sensor(0x0000));
			if (*sensor_id ==imgsensor_info.sensor_id) {
				pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}
			pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor Read sensor id fail, id: 0x%x\n",*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor PLATFORM:MT6797,MIPI 4LANfunc = %s \n",__func__);
...
static kal_uint32 get_info(enum MSDK_SCENARIO_ID_ENUM scenario_id,
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor scenario_id = %d\n", scenario_id);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	switch (scenario_id) {
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			break;
		default:
			pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor Error ScenarioId setting");
			preview(image_window, sensor_config_data);
...
{
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor framerate = %d\n ", framerate);
...
{
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	switch (scenario_id) {
        ...
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
        	  if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
                ...
                spin_lock(&imgsensor_drv_lock);
                    ...
                    pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",framerate,imgsensor_info.cap.max_framerate/10);
		...
		default:  //coding with  preview scenario by default
			...
			spin_unlock(&imgsensor_drv_lock);
			//set_dummy();
			pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor error scenario_id = %d, we use preview scenario \n", scenario_id);
			break;
	}
	return ERROR_NONE;
}
...
{
	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor enable: %d\n", enable);
...
//prize add by lipengpeng 20210809 end 
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			//pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor SENSOR_FEATURE_GET_PDAF_DATA\n");
		...
		//	pr_debug(" liaojie s5k3p3sxgmsmipiraw_Sensor SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%lld\n", *feature_data);
