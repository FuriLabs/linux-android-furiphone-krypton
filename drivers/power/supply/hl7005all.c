...
/*prize add by zhaopengge----20201030-------start*/
	...
	case EVENT_RECHARGE:
		...
		break;
/*prize add by zhaopengge----20201030-------end*/
	default:
		break;
	}
...
static int hl7005_enable_chip(struct charger_device *chg_dev, bool en)
{
	...
	if (en) {
		if (((struct hl7005all_info *)chg_dev->dev.driver_data)->dev_model == DEVICE_ETA6937){
			...
			hl7005_reg_config_interface(0x06, 0xAB);//set vsafe 4.42 ,actually 4.4 due to deviation 	//safe set to 4.4v
...
static int hl7005_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	switch(info->dev_model){
		...
		case DEVICE_ETA6937:{
			hl7005_reg_config_interface(0x06, 0xAB);//set vsafe 4.42 ,actually 4.4 due to deviation
