...
#define LOG_INF(format, args...)       pr_debug(MyTag "[%s] " format, \
	 __func__, ##args)
#define LOG_NOTICE(format, args...)    pr_debug(MyTag "[%s] " format, \
	 __func__, ##args)
...
#define IRQ_LOG_KEEPER(irq, ppb, logT, fmt, ...) do {\
	...
	if (avaLen > 1) {\
		if (snprintf((char *)(pDes), avaLen, "[%d.%06d]" fmt,\
			...
			##__VA_ARGS__) < 0) {\
			pr_debug("[Error] %s: snprintf failed", __func__);\
		} \
		if ('\0' != gSvLog[irq]._str[ppb][logT][str_leng - 1]) {\
			pr_debug("log str over flow(%d)", irq);\
		} \
		while (*ptr++ != '\0') {        \
			(*ptr2)++;\
		}     \
	} else { \
		pr_debug("(%d)(%d)log str avalible=0, print log\n", irq, logT);\
		...
		if (pSrc->_cnt[ppb][logT] != 0) {\
			...
			if (snprintf((char *)(pDes),\
				...
				pr_debug("[Error] %s: snprintf failed",\
					__func__);\
			} \
			while (*ptr++ != '\0') {\
				(*ptr2)++;\
...
{
	pr_debug("ERROR:CAM_REG_CTL_EN is secure protect region can't read/write in camera-isp\n");
	return NULL;
}

static inline void *SecureError_DMA_EN(void)
{
	pr_debug("ERROR:CAM_REG_CTL_DMA_EN is secure protect region can't read/write in camera-isp\n");
	return NULL;
}
...
{
	pr_debug("ERROR:CAM_REG_CTL_SEL is secure protect region can't read/write in camera-isp\n");
	return NULL;
}
...
static signed int ISP_DumpSeninfReg(void)
{
	...
	/* IMGPLL frequency */
	pr_debug("IMGPLL frequency(0x%x)[HPM:0x114EC5, LPM:0xC8000]",
...
static signed int ISP_DumpReg(void)
{
	...
	pr_debug("[0x%08X %08X] [0x%08X %08X]",
	...
	/* hbin */
	pr_debug("[%s] [0x%08X %08X],[0x%08X %08X]",
	...
	pr_debug("[0x%08X %08X]", (unsigned int)(ISP_TPIPE_ADDR + 0x43B8),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x43B8));
	pr_debug("[0x%08X %08X]", (unsigned int)(ISP_TPIPE_ADDR + 0x43BC),
	...
	pr_debug("[0x%08X %08X]", (unsigned int)(ISP_TPIPE_ADDR + 0x4BB8),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4BB8));
	pr_debug("[0x%08X %08X]", (unsigned int)(ISP_TPIPE_ADDR + 0x4BBC),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x43BC));
	...
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x800),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x800));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x880),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x880));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x884),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x884));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x888),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x888));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x8A0),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x8A0));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x920),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x920));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x924),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x924));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x928),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x928));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x92C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x92C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x930),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x930));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x934),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x934));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x938),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x938));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x93C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x93C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x960),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x960));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x9C4),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x9C4));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x9E4),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x9E4));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x9E8),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x9E8));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x9EC),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x9EC));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xA00),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xA00));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xA04),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xA04));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xA08),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xA08));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xA0C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xA0C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xA10),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xA10));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xA14),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xA14));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xA20),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xA20));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xAA0),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xAA0));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xACC),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xACC));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB00),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB00));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB04),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB04));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB08),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB08));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB0C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB0C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB10),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB10));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB14),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB14));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB18),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB18));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB1C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB1C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB20),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB20));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB44),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB44));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB48),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB48));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB4C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB4C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB50),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB50));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB54),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB54));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB58),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB58));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB5C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB5C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xB60),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xB60));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xBA0),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xBA0));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xBA4),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xBA4));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xBA8),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xBA8));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xBAC),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xBAC));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xBB0),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xBB0));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xBB4),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xBB4));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xBB8),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xBB8));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xBBC),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xBBC));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xBC0),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xBC0));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xC20),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xC20));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xCC0),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xCC0));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xCE4),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xCE4));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xCE8),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xCE8));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xCEC),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xCEC));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xCF0),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xCF0));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xCF4),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xCF4));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xCF8),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xCF8));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xCFC),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xCFC));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xD24),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xD24));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xD28),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xD28));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xD2C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xD2c));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xD40),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xD40));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xD64),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xD64));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xD68),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xD68));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xD6C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xD6c));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xD70),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xD70));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xD74),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xD74));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xD78),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xD78));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xD7C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xD7C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xDA4),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xDA4));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xDA8),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xDA8));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0xDAC),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xDAC));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x2410),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x2410));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x2414),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x2414));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x2418),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x2418));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x241C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x241C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x2420),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x2420));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x243C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x243C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x2440),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x2440));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x2444),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x2444));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x2448),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x2448));

	/* seninf3 */
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4900),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4900));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4920),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4920));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4924),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4924));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4928),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4928));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x492C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x492C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4930),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4930));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4934),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4934));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4938),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4938));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4BA0),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4BA0));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4BA4),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4BA4));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4BA8),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4BA8));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4BAC),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4BAC));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4BB0),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4BB0));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4BB4),
	...
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4BBC));
	/* seninf4 */
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4D00),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4D00));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4D20),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4D20));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4D24),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4D24));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4D28),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4D28));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4D2C),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4D2C));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4D30),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4D30));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4D34),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4D34));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4D38),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4D38));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4FA0),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4FA0));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4FA4),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4FA4));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4FA8),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4FA8));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4FAC),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4FAC));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4FB0),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4FB0));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4FB4),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4FB4));
	ISP_WR32((ISP_ADDR + 0x4FB8), 0x10);
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4FB8),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x4FB8));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x4FBC),
	...
	pr_debug("0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR + 0x35FC),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x35FC));
	pr_debug("end MT6593");

	/*  */
	pr_debug("0x%08X %08X ", (unsigned int)ISP_ADDR_CAMINF,
	(unsigned int)ISP_RD32(ISP_ADDR_CAMINF));
	pr_debug("0x%08X %08X ", (unsigned int)(ISP_TPIPE_ADDR + 0x150),
	...
	/* mdp crop */
	pr_debug("MDPCROP Related");
	pr_debug("0x%08X %08X", (unsigned int)(ISP_ADDR + 0xd10),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xd10));
	pr_debug("0x%08X %08X", (unsigned int)(ISP_ADDR + 0xd20),
	(unsigned int)ISP_RD32(ISP_ADDR + 0xd20));
	/* cq */
	pr_debug("CQ Related");
	ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x6000);
	pr_debug("0x%08X %08X (0x15004160=6000)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x7000);
	pr_debug("0x%08X %08X (0x15004160=7000)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x8000);
	pr_debug("0x%08X %08X (0x15004160=8000)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	/* imgi_debug */
	pr_debug("IMGI_DEBUG Related");
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x001e);
	pr_debug("0x%08X %08X (0x150075f4=001e)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x011e);
	pr_debug("0x%08X %08X (0x150075f4=011e)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x021e);
	pr_debug("0x%08X %08X (0x150075f4=021e)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x031e);
	pr_debug("0x%08X %08X (0x150075f4=031e)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	/* yuv */
	pr_debug("yuv-mdp crop Related");
	ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x3014);
	pr_debug("0x%08X %08X (0x15004160=3014)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	pr_debug("yuv-c24b out Related");
	ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x301e);
	pr_debug("0x%08X %08X (0x15004160=301e)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x301f);
	pr_debug("0x%08X %08X (0x15004160=301f)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x3020);
	pr_debug("0x%08X %08X (0x15004160=3020)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0x3021);
	pr_debug("0x%08X %08X (0x15004160=3021)",
	(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
	(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
...
#define TPIPE_DUMP_SIZE    200
		...
		if ((ctlStart & 0x01) && (tpipePA) && (ctlTcm & 0x80000000)) {
			...
			m4u_mva_map_kernel(tpipePA, TPIPE_DUMP_SIZE, 0,
						&map_va, &map_size);
			pMapVa = map_va;
			pr_debug("pMapVa(0x%x),map_size(0x%x)",
				pMapVa, map_size);
			pr_debug("ctlStart(0x%x),tpipePA(0x%x),ctlTcm(0x%x)",
...
static inline void Prepare_Enable_ccf_clock(void)
{
	...
	/* pr_debug("enable CG/MTCMOS through SMI CLK API\n"); */
...
static inline void Disable_Unprepare_ccf_clock(void)
{
	...
	#ifndef EP_MARK_SMI
	/* pr_debug("disable CG/MTCMOS through SMI CLK API\n"); */
...
static inline void Prepare_Enable_cg_clock(void)
{
	...
	#ifndef EP_MARK_SMI
	pr_debug("enable CG through SMI CLK API\n");
	smi_bus_prepare_enable(SMI_LARB3, ISP_DEV_NAME);
	#endif
...
static inline void Disable_Unprepare_cg_clock(void)
{
	...
	#ifndef EP_MARK_SMI
	pr_debug("disable CG through SMI CLK API\n");
	smi_bus_disable_unprepare(SMI_LARB3, ISP_DEV_NAME);
	#endif
}
...
void ISP_Halt_Mask(unsigned int isphaltMask)
{
	...
	pr_debug("ISP halt_en for dvfs:0x%x\n",
...
static void ISP_EnableClock(bool En)
{
	...
	if (En) {
		...
		/* pr_debug("Camera clock enbled. G_u4EnableClockCount: %d.",
		...
		case 0:
			...
			/*pr_debug("MTK_LEGACY:enable clk");*/
...
#else/*CCF*/
		/*pr_debug("CCF:prepare_enable clk");*/
		spin_lock(&(IspInfo.SpinLockClock));
	...
	ISP_WR32(ISP_CAMSYS_CONFIG_BASE + 0x120, 0xFFFFFF4F);
...
#if defined(EP_NO_CLKMGR) || defined(CONFIG_MTK_CLKMGR)
		spin_lock(&(IspInfo.SpinLockClock));
		/* pr_debug("Camera clock disabled. G_u4EnableClockCount: %d.",
		 *         G_u4EnableClockCount);
		 */
		G_u4EnableClockCount--;
		switch (G_u4EnableClockCount) {
...
#else
			/*pr_debug("MTK_LEGACY:disable clk");*/
			/* do disable clock     */
		...
		spin_unlock(&(IspInfo.SpinLockClock));
#else
		/*pr_debug("CCF:disable_unprepare clk\n");*/
		spin_lock(&(IspInfo.SpinLockClock));
		if (G_u4EnableClockCount == 0) {
			spin_unlock(&(IspInfo.SpinLockClock));
			pr_debug(
...
static inline void ISP_Reset(signed int module)
{
	...
	case ISP_CAM_B_IDX: {
		...
		while (ISP_RD32(CAM_REG_CTL_SW_CTL(module)) != 0x2)
			pr_debug("CAM resetting... module:%d\n", module);
#else
		mdelay(1);
#endif
	...
	case ISP_CAMSV5_IDX: {
		...
		while (ISP_RD32(CAMSV_REG_SW_CTL(module)) != 0x3)
			pr_debug("CAMSV resetting... module:%d\n", module);
	...
	case ISP_UNI_A_IDX: {
		...
		while (ISP_RD32(CAM_UNI_REG_TOP_SW_CTL(module)) != 0x222)
			pr_debug("UNI resetting... module:%d\n", module);
#else
		mdelay(1);
#endif
...
#endif
		pr_debug("copy dumpbuf::0x%p tpipebuf:0x%p is done!!\n",
...
#endif
		pr_debug("copy dumpbuf::0x%p tuningbuf:0x%p is done!!\n",
...
#endif
		pr_debug("copy dumpbuf::0x%p virispbuf:0x%p is done!!\n",
...
#endif
		pr_debug("copy dumpbuf::0x%p cmdqbuf:0x%p is done!!\n",
...
static signed int ISP_SetMemInfo(struct ISP_MEM_INFO_STRUCT *pMemInfoStruct)
{
	...
	case ISP_MEMORY_INFO_TPIPE_CMD:
		...
		pr_debug("set tpipe memory info is done!!\n");
		break;
...
static long ISP_REF_CNT_CTRL_FUNC(unsigned long Param)
{
	...
	/* pr_debug("[rc]+ QQ"); */ /* for memory corruption check */
	...
	if (copy_from_user(&ref_cnt_ctrl, (void __user *)Param,
		...
		if (ref_cnt_ctrl.id < ISP_REF_CNT_ID_MAX) {
			...
			/*  */
			if (IspInfo.DebugMask & ISP_DBG_REF_CNT_CTRL)
				pr_debug("[rc]ref_cnt(%d)", imem_ref_cnt);
	...
	/* pr_debug("[rc]QQ return value:(%d)", Ret); */
	/*  */
	/* //////////////////---add unlock here */
	/* spin_unlock_irqrestore(&(IspInfo.SpinLock), flags); */
	/* ////////////////// */
	...
	if (copy_from_user(&rt_buf_ctrl, (void __user *)Param,
		...
		case ISP_RT_BUF_CTRL_CLEAR:
			...
			if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL)
				pr_debug("[rtbc][%d][CLEAR]:rt_dma(%d)\n",
		...
		enum ISP_P2_BUFQUE_MATCH_TYPE type, signed int *idx)
{
	...
	case ISP_P2_BUFQUE_MATCH_TYPE_WAITFMEQD:
		spin_lock(&(SpinLock_P2FrameList));
		/* pr_debug("check bf(%d_0x%x_0x%x/%d_%d)", param.property,
		...
		if (index == -1) {
			/* pr_debug("check bf(%d_0x%x_0x%x / %d_%d) ",
			 * param.property,
			 * param.processID, param.callerID,index, *idx);
			 */
			spin_unlock(&(SpinLock_P2FrameList));
...
static signed int ISP_P2_BufQue_CTRL_FUNC(struct ISP_P2_BUFQUE_STRUCT param)
{
	...
	case ISP_P2_BUFQUE_CTRL_DEQUE_SUCCESS:
		if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL)
			pr_debug("dq cm(%d),pID(0x%x),cID(0x%x)\n",
	...
	case ISP_P2_BUFQUE_CTRL_WAIT_FRAME:/* wait for a specific buffer */
...
#ifdef P2_DBG_LOG
		pr_debug("ISP_P2_BUFQUE_CTRL_WAIT_FRAME, after pty/pID/cID (%d/0x%x/0x%x),idx(%d)",
		...
		}
		{
			spin_unlock(&(SpinLock_P2FrameList));
			if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL)
				pr_debug("=pd(%d/0x%x/0x%x_%d)wait(%d s)=\n",
...
static signed int ISP_WaitIrq(struct ISP_WAIT_IRQ_STRUCT *WaitIrq)
{
	...
	if ((WaitIrq->EventInfo.St_type >= ISP_IRQ_ST_AMOUNT) ||
		...
		pr_debug("invalid St_type(%d), max(%d)\n",
	...
	if (IspInfo.DebugMask & ISP_DBG_INT) {
		if (WaitIrq->EventInfo.Status &
			...
			if (WaitIrq->EventInfo.UserKey > 0) {
				pr_debug("+WaitIrq Clear(%d), Type(%d), Status(0x%08X), Timeout(%d/%d),user(%d)\n",
...
#ifdef ENABLE_WAITIRQ_LOG
	pr_debug("before wait: Clear(%d) Type(%d) StType(%d) Sts(0x%08X) WaitSts(0x%08X) Timeout(%d) userKey(%d)\n",
		WaitIrq->EventInfo.Clear,
		WaitIrq->Type,
		WaitIrq->EventInfo.St_type,
		irqStatus,
...
static void ISP_ion_init(void)
{
	...
	if (IspInfo.DebugMask & ISP_DBG_ION_CTRL)
		pr_debug("create ion client 0x%pK\n", pIon_client);
}
...
static void ISP_ion_uninit(void)
{
	...
	if (IspInfo.DebugMask & ISP_DBG_ION_CTRL)
		pr_debug("destroy ion client 0x%pK\n", pIon_client);
						...
						int fd)
{
	...
	if (IspInfo.DebugMask & ISP_DBG_ION_CTRL)
		pr_debug("[ion_import_hd] Hd(0x%pK)\n", handle);
	return handle;
}
...
static void ISP_ion_free_handle_by_module(unsigned int module)
{
	...
	if (IspInfo.DebugMask & ISP_DBG_ION_CTRL)
		pr_debug("[ion_free_hd_by_module]%d\n", module);
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case ISP_WAKELOCK_CTRL:
		if (copy_from_user(&wakelock_ctrl, (void *)Param,
			...
			if (wakelock_ctrl == 1) {/* Enable wakelock */
...
#endif
					g_WaitLockCt++;
					pr_debug("wakelock enable!! cnt(%d)\n",
						g_WaitLockCt);
				}
				...
				if (g_WaitLockCt)
					pr_debug("subtract wakelock cnt(%d)\n",
						g_WaitLockCt);
				else {
#ifdef CONFIG_PM_SLEEP
					__pm_relax(isp_wake_lock);
#endif
					pr_debug("wakelock disable!! cnt(%d)\n",
						g_WaitLockCt);
				}
			}
		}
	...
	case ISP_DEBUG_FLAG:
		if (copy_from_user(DebugFlag, (void *)Param,
			...
			/* pr_debug("FBC kernel debug level = %x\n",
			 *          IspInfo.DebugMask);
			 */
		} else {
			LOG_NOTICE("copy_from_user failed\n");
	...
	case ISP_VF_LOG:
		if (copy_from_user(DebugFlag, (void *)Param,
				...
				case ISP_CAM_A_IDX:
					...
					hds2_sel = ((ISP_RD32(
						CAM_UNI_REG_TOP_PATH_SEL(
						  ISP_UNI_A_IDX))) & 0x3);
					pr_debug("CAM_A viewFinder is ON (SecOn:0x%x)\n",
						sec_on);
				...
				case ISP_CAM_B_IDX:
					...
					hds2_sel = ((ISP_RD32(
						CAM_UNI_REG_B_TOP_PATH_SEL(
							ISP_UNI_A_IDX))) & 0x3);
					pr_debug("CAM_B viewFinder is ON\n");
					cam_dmao = ISP_RD32(CAM_REG_CTL_DMA_EN(
							ISP_CAM_B_IDX));
					pr_debug("CAM_B:[DMA_EN]:0x%x\n",
						cam_dmao);
					vf = ISP_RD32(CAM_REG_TG_VF_CON(
						ISP_CAM_B_IDX));
					if (vf & 0x1)
			...
			case 0: {
				switch (DebugFlag[1]) {
				case ISP_CAM_A_IDX:
					pr_debug("CAM_A viewFinder is OFF\n");
					vf = ISP_RD32(CAM_REG_TG_VF_CON(
							ISP_CAM_A_IDX));
					if (vf & 0x1) {
						ISP_WR32(CAM_REG_TG_VF_CON(
					...
					break;
				case ISP_CAM_B_IDX:
					pr_debug("CAM_B viewFinder is OFF\n");
					vf = ISP_RD32(CAM_REG_TG_VF_CON(
							ISP_CAM_B_IDX));
					if (vf & 0x1) {
						ISP_WR32(CAM_REG_TG_VF_CON(
			...
			case 11: {
				...
				switch (DebugFlag[1]) {
				case ISP_CAMSV0_IDX:
					pr_debug("CAMSV_0 viewFinder is ON\n");
					cam_dmao = (ISP_RD32(CAMSV_REG_MODULE_EN
						(ISP_CAMSV0_IDX)) & 0x10);
					pr_debug("CAMSV_0:[DMA_EN]:0x%x\n",
						cam_dmao);
					...
					break;
				case ISP_CAMSV1_IDX:
					pr_debug("CAMSV_1 viewFinder is ON\n");
					cam_dmao = (ISP_RD32(CAMSV_REG_MODULE_EN
						(ISP_CAMSV1_IDX)) & 0x10);
					pr_debug("CAMSV_1:[DMA_EN]:0x%x\n",
						cam_dmao);
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(
						ISP_CAMSV1_IDX));
					if (vf & 0x1) {
					...
					break;
				case ISP_CAMSV2_IDX:
					pr_debug("CAMSV_2 viewFinder is ON\n");
					cam_dmao = (ISP_RD32(CAMSV_REG_MODULE_EN
						(ISP_CAMSV2_IDX)) & 0x10);
					pr_debug("CAMSV_2:[DMA_EN]:0x%x\n",
						cam_dmao);
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(
						ISP_CAMSV2_IDX));
					if (vf & 0x1) {
					...
					break;
				case ISP_CAMSV3_IDX:
					pr_debug("CAMSV_3 viewFinder is ON\n");
					cam_dmao = (ISP_RD32(CAMSV_REG_MODULE_EN
						(ISP_CAMSV3_IDX)) & 0x10);
					pr_debug("CAMSV_3:[DMA_EN]:0x%x\n",
						cam_dmao);
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(
							ISP_CAMSV3_IDX));
					if (vf & 0x1) {
					...
					break;
				case ISP_CAMSV4_IDX:
					pr_debug("CAMSV_4 viewFinder is ON\n");
					cam_dmao = (ISP_RD32(CAMSV_REG_MODULE_EN
						(ISP_CAMSV4_IDX)) & 0x10);
					pr_debug("CAMSV_4:[DMA_EN]:0x%x\n",
						cam_dmao);
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(
							ISP_CAMSV4_IDX));
					if (vf & 0x1) {
					...
					break;
				case ISP_CAMSV5_IDX:
					pr_debug("CAMSV_5 viewFinder is ON\n");
					cam_dmao = (ISP_RD32(CAMSV_REG_MODULE_EN
						(ISP_CAMSV5_IDX)) & 0x10);
					pr_debug("CAMSV_5:[DMA_EN]:0x%x\n",
						cam_dmao);
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(
							ISP_CAMSV5_IDX));
					if (vf & 0x1) {
			...
			case 10: {
				switch (DebugFlag[1]) {
				case ISP_CAMSV0_IDX:
					pr_debug("CAMSV_0 viewFinder is OFF\n");
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(
							ISP_CAMSV0_IDX));
					if (vf & 0x1) {
						ISP_WR32(CAMSV_REG_TG_VF_CON(
					...
					break;
				case ISP_CAMSV1_IDX:
					pr_debug("CAMSV_1 viewFinder is OFF\n");
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(
							ISP_CAMSV1_IDX));
					if (vf & 0x1) {
						ISP_WR32(CAMSV_REG_TG_VF_CON(
					...
					break;
				case ISP_CAMSV2_IDX:
					pr_debug("CAMSV_2 viewFinder is OFF\n");
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(
							ISP_CAMSV2_IDX));
					if (vf & 0x1) {
						ISP_WR32(CAMSV_REG_TG_VF_CON(
					...
					break;
				case ISP_CAMSV3_IDX:
					pr_debug("CAMSV_3 viewFinder is OFF\n");
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(
							ISP_CAMSV3_IDX));
					if (vf & 0x1) {
						ISP_WR32(CAMSV_REG_TG_VF_CON(
					...
					break;
				case ISP_CAMSV4_IDX:
					pr_debug("CAMSV_4 viewFinder is OFF\n");
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(
							ISP_CAMSV4_IDX));
					if (vf & 0x1) {
						ISP_WR32(CAMSV_REG_TG_VF_CON(
					...
					break;
				case ISP_CAMSV5_IDX:
					pr_debug("CAMSV_5 viewFinder is OFF\n");
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(
							ISP_CAMSV5_IDX));
					if (vf & 0x1) {
						ISP_WR32(CAMSV_REG_TG_VF_CON(
	...
	case ISP_GET_SUPPORTED_ISP_CLOCKS:
		...
		pr_debug("Default DFS Clk level:%d for EP",
			ispclks.clklevel[lv]);
			...
			unsigned long arg)
{
	...
	case COMPAT_ISP_READ_REGISTER: {
		...
		if (err) {
			pr_debug("compat_get_isp_read_register_data error!!!\n");
			return err;
		}
		...
		if (err) {
			pr_debug("compat_put_isp_read_register_data error!!!\n");
			return err;
		}
		return ret;
	}
	case COMPAT_ISP_WRITE_REGISTER: {
		...
		err = compat_get_isp_read_register_data(data32, data);
		if (err) {
			pr_debug("COMPAT_ISP_WRITE_REGISTER error!!!\n");
			return err;
		}
	...
	case COMPAT_ISP_BUFFER_CTRL: {
		...
		if (err) {
			pr_debug("compat_get_isp_buf_ctrl error!!!\n");
			return err;
		}
		...
		if (err) {
			pr_debug("compat_put_isp_buf_ctrl error!!!\n");
			return err;
		}
		return ret;
	...
	case COMPAT_ISP_REF_CNT_CTRL: {
		...
		if (err) {
			pr_debug("compat_get_isp_ref_cnt_ctrl error!!!\n");
			return err;
		}
		...
		if (err) {
			pr_debug("compat_put_isp_ref_cnt_ctrl_ error!!!\n");
			return err;
		}
		return ret;
	}
	...
	case COMPAT_ISP_REGISTER_IRQ_USER_KEY: {
		...
		if (err) {
			pr_debug("compat_get_isp_register_userkey error!!!\n");
			return err;
		}
		...
		if (err) {
			pr_debug("compat_put_isp_register_userkey error!!!\n");
			return err;
		}
		return ret;
	}
	...
	case COMPAT_ISP_DUMP_BUFFER: {
		...
		if (err) {
			pr_debug("COMPAT_ISP_DUMP_BUFFER error!!!\n");
			return err;
		}
	...
	case COMPAT_ISP_SET_MEM_INFO: {
		...
		if (err) {
			pr_debug("COMPAT_ISP_SET_MEM_INFO error!!!\n");
			return err;
		}
	...
	struct file *pFile)
{
	...
	if (IspInfo.BufInfo.Read.pData == NULL) {
		pr_debug("ERROR: BufRead kmalloc failed\n");
		Ret = -ENOMEM;
		goto EXIT;
	}
...
EXIT:
	...
	pr_debug("- X. Ret: %d. UserCount: %d. G_u4EnableClockCount:%d\n", Ret,
...
static inline void ISP_StopHW(signed int module)
{
	...
	if (-ERESTARTSYS == ret) {
		pr_debug("%s: interrupt by system signal, wait idle\n",
			moduleName);
		/* timer*/
		m_sec = ktime_get();

		if (regTGSt == 1)
			pr_debug("%s: wait idle done\n", moduleName);
		else if (regTGSt == 0)
			pr_debug("plz check regTGSt value");
		else
			pr_debug("%s: wait idle timeout(%lld)\n", moduleName,
				(sec - m_sec));
	}
	...
	if (-EFAULT == ret || regTGSt != 1) {
		...
		while (ISP_RD32(CAM_REG_CTL_SW_CTL(module)) != 0x2) {
			...
			if ((sec  - m_sec) > timeoutMs) {
				pr_debug("%s: wait SW idle timeout\n",
					moduleName);
				break;
			}
		}
		...
		regTGSt = (ISP_RD32(CAM_REG_TG_INTER_ST(module)) & 0x00003F00)
			   >> 8;
		pr_debug("%s_TG_ST(%d)_SW_ST(0x%x)\n", moduleName, regTGSt,
	...
	struct file *pFile)
{
	...
	/*  */
	pr_debug("Start ISP_StopHW");
	ISP_StopHW(ISP_CAM_A_IDX);
	ISP_StopHW(ISP_CAM_B_IDX);
	pr_debug("End ISP_StopHW");
	...
	pr_debug("Start ISP_ion_uninit");
	ISP_ion_uninit();
	pr_debug("End ISP_ion_uninit");
#endif

	/*  */
	/* pr_debug("Before spm_enable_sodi()."); */
...
EXIT:
	mutex_unlock(&gDipMutex);
	pr_debug("- X. UserCount: %d. G_u4EnableClockCount:%d",
		IspInfo.UserCount, G_u4EnableClockCount);
	return 0;
}
...
static signed int ISP_probe(struct platform_device *pDev)
{
	...
	pr_debug("nr_isp_devs=%d, devnode(%s), map_addr=0x%lx\n",
		nr_isp_devs, pDev->dev.of_node->name,
	...
	if (isp_dev->irq > 0) {
		...
		pr_debug("No IRQ!!: nr_isp_devs=%d, devnode(%s), irq=%d\n",
...
)
{
	...
	if (!G_u4EnableClockCount) {
		...
		if (strcmp(moduleName,
			...
			pr_debug("%s - X. UserCount=%d,wakelock:%d,devct:%d\n",
	...
	/*pr_debug("%s: Rs_TG(0x%08x)\n", moduleName, regVal);*/

	if (regVal & 0x01) {
		pr_debug("%s_suspend,disable VF,wakelock:%d,clk:%d,devct:%d\n",
		...
		pr_debug("%s_suspend,wakelock:%d,clk:%d,devct:%d\n",
...
EXIT:
	...
	if (!atomic_read(&G_u4DevNodeCt)) {
		...
		pr_debug("%s - X. wakelock:%d, last dev node,disable clk:%d\n",
...
static int ISP_resume(struct platform_device *pDev)
{
	...
	if (SuspnedRecord[module]) {
		pr_debug("%s_resume,enable VF,wakelock:%d,clk:%d,devct:%d\n",
			moduleName, g_WaitLockCt, G_u4EnableClockCount,
			atomic_read(&G_u4DevNodeCt));
		SuspnedRecord[module] = 0;
		...
		pr_debug("%s_resume,wakelock:%d,clk:%d,devct:%d\n", moduleName,
...
static int isp_p2_ke_dump_read(struct seq_file *m, void *v)
{
	...
	pr_debug("isp p2 ke dump start!! g_bDumpPhyISPBuf:%d\n",
		g_bDumpPhyISPBuf);
	pr_debug("g_bDumpPhyISPBuf:%d, g_tdriaddr:0x%x, g_cmdqaddr:0x%x\n",
	...
	if (g_pPhyISPBuffer != NULL) {
		...
		pr_debug("g_pPhyISPBuffer:(0x%pK)\n", g_pPhyISPBuffer);
	}
...
static signed int __init ISP_Init(void)
{
	...
	do {
		...
		for (i = 0; i < ARRAY_SIZE(SMI_LARB_BASE); i++) {
			...
			pr_debug("Finding SMI_LARB compatible: %s\n", comp_str);
	...
	pr_debug("register isp callback for MDP");
	...
	pr_debug("register isp callback for GCE");
...
static void __exit ISP_Exit(void)
{
	...
	pr_debug("Un-register isp callback for GCE");
...
{
	/* pr_debug("ISP_MDPClockOnCallback"); */
	/*pr_debug("+MDPEn:%d", G_u4EnableClockCount);*/
	ISP_EnableClock(MTRUE);
...
{
	/* pr_debug("ISP_MDPClockOffCallback"); */
	ISP_EnableClock(MFALSE);
	/*pr_debug("-MDPEn:%d", G_u4EnableClockCount);*/
	return 0;
}
		...
		uint32_t *regValues)
{
	...
	pr_debug("End taskID(%d),regCount(%d)", taskID, regCount);
	...
	for (i = 0; i < regCount; i += PER_LINE_LOG_SIZE) {
		...
		pr_debug("[0x%08x,0x%08x][0x%08x,0x%08x][0x%08x,0x%08x][0x%08x,0x%08x][0x%08x,0x%08x]\n",
	...
	pr_debug("kk:tpipePA(0x%x), ctlStart(0x%x)", tpipePA, ctlStart);
	...
	if ((tpipePA)) {
		...
		if (pMapVa) {
			for (i = 0; i < TPIPE_DUMP_SIZE; i += 10) {
				pr_debug("[idx(%d)]%08X-%08X-%08X-%08X-%08X-%08X-%08X-%08X-%08X-%08X",
...
static signed int DMAErrHandler(void)
{
	...
	pr_debug("err_ctrl(0x%08x)", err_ctrl);
	...
	for (i = 0; i < 12; i++) {
		...
		if (err_ctrl & (*pErr)) {
			...
			pr_debug("(0x%08x, 0x%08x), dbg(0x%08x, 0x%08x)",
				addr, ISP_RD32(addr),
...
static int32_t ISP_WaitTimestampReady(unsigned int module, unsigned int dma_id)
{
	...
	pr_debug("Wait module:%d dma:%d timestamp ready W/R:%d/%d\n",
	    module, dma_id,
	...
	for (wait_cnt = 3; wait_cnt > 0; wait_cnt--) {
		...
		pr_debug("WARNING: cam:%d dma:%d wait left count %d\n",
...
if ((wait_cnt & 0x7) == 0x7)
	pr_debug("WARNING: module:%d dma:%d wait long %d W/R:%d/%d\n",
		module, dma_id, wait_cnt,
		(unsigned int)
		IspInfo.TstpQInfo[module].Dmao[dma_id].TotalWrCnt,
		(unsigned int)
...
if (wait_cnt > 3000) {
	pr_debug("ERROR: module:%d dma:%d wait timeout %d W/R:%d/%d\n",
		module, dma_id, wait_cnt,
		(unsigned int)
		IspInfo.TstpQInfo[module].Dmao[dma_id].TotalWrCnt,
		(unsigned int)
			...
			unsigned int frmPeriod)
{
	...
	if (delta_wcnt > 255) {
		...
		if (dmao_mask)
			pr_debug("WARNING: Cam:%d dma:%d SUSPICIOUS WCNT: %d_%d_%d\n",
			    module, dma_id, delta_wcnt,
			    IspInfo.TstpQInfo[module].Dmao[dma_id].PrevFbcWCnt,
			    fbc_ctrl2.Bits.WCNT);
...
time_prev2.usec =
	...
	if ((sec > time_prev1.sec) ||
		...
		if (dmao_mask)
			pr_debug("ERROR: Cam:%d dma:%d current timestamp: cur: %d.%06d prev1: %d.%06d\n",
		...
		unsigned long long prevTimestp)
{
	...
	if (frm_dt == 0)
		pr_debug("WARNING: timestamp delta too small: %d\n",
...
irqreturn_t ISP_Irq_DIP_A(signed int  Irq, void *DeviceId)
{
	...
	/*pr_debug("ISP_Irq_DIP_A:%d, reg 0x%p : 0x%x, reg 0x%p : 0x%x\n",
...
static void SMI_INFO_DUMP(enum ISP_IRQ_TYPE_ENUM irq_module)
{
	...
	case ISP_IRQ_TYPE_INT_CAM_B_ST:
		if (g_ISPIntErr_SMI[irq_module] &
			...
			pr_debug("ERR:SMI_DUMP by module:%d\n", irq_module);
