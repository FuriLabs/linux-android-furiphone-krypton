...
#ifdef FSM_VBAT_MONITOR

#define VBAT_LEVEL_1  (3900)
#define VBAT_LEVEL_2  (3800)
#define VBAT_LEVEL_3  (3700)
#define VBAT_LEVEL_4  (3600)

struct power_supply *psy;
struct delayed_work monitor_work;
struct workqueue_struct *wq;

int fsm_get_amp_tempr(int *amp_data)
{
...
int fsm_set_monitor(bool enable)
{
	if (enable) {
		...
		if (delayed_work_pending(&monitor_work)) {
...
static void fsm_work_monitor(struct work_struct *work)
{
	...
	fsm_mutex_lock();
#ifdef FSM_VBAT_MONITOR
	fsm_get_amp_tempr(amp_data);
	if(amp_data[0] > VBAT_LEVEL_1){
		fsm_set_scene(0);
	}else if((amp_data[0] < VBAT_LEVEL_1)&&(amp_data[0] > VBAT_LEVEL_2)){
		fsm_set_scene(1);
	}else if((amp_data[0] < VBAT_LEVEL_2)&&(amp_data[0] > VBAT_LEVEL_3)){
		fsm_set_scene(2);
	}else if((amp_data[0] < VBAT_LEVEL_3)&&(amp_data[0] > VBAT_LEVEL_4)){
		fsm_set_scene(3);
	}else{
		fsm_set_scene(4);
	}
	...
	fsm_mutex_unlock();
	/* reschedule */
	queue_delayed_work(wq, &monitor_work,
			5*HZ);
}
			...
			const struct i2c_device_id *id)
{
	...
	wq = create_singlethread_workqueue("fs16xx");
	INIT_DELAYED_WORK(&monitor_work, fsm_work_monitor);
#endif
