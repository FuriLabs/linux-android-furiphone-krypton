...
/*
 * AW8601CSR voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "AW8601CSR_DRV"
...
static int s4AW8601CSR_ReadReg(unsigned short *a_pu2Result)
{
 ...
 * I2c read/write
 ******************************************************************************/
static int AW8601CSR_WriteReg(unsigned char a_uAddr, unsigned char a_uData)
{
	int ret = 0;
...
static int AW8601CSR_ReadReg(unsigned char a_uAddr, unsigned char *a_puData)
{
	int ret = 0;
	char a_uResult;
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		initdrv();
		ret = s4AW8601CSR_ReadReg(&InitPos);
...
long AW8601CSR_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	long i4RetValue = 0;
...
int AW8601CSR_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
...
int AW8601CSR_PowerDown(struct i2c_client *pstAF_I2Cclient, int *pAF_Opened)
{
	unsigned char data;
	...
	AW8601CSR_WriteReg(0x08, 0x0a);

	AW8601CSR_ReadReg(0x02, &data);
	data |= 0x01;
	AW8601CSR_WriteReg(0x02, data); /* enter PD mode, PD = 1; */
...
int AW8601CSR_GetFileName(unsigned char *pFileName)
{
