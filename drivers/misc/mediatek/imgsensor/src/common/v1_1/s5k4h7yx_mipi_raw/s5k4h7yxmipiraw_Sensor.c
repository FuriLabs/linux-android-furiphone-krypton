...
//prize add by lipengpeng 20211209 start 
	.min_gain = 73,
	.max_gain = 4096,
	.min_gain_iso = 100,
	.exp_step = 2,
	.gain_step = 1,
	.gain_type = 0,
//prize add by lipengpeng 20211209 end		
	.margin = 4,
	.min_shutter = 4,
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gr,  //SENSOR_OUTPUT_FORMAT_RAW_Gb SENSOR_OUTPUT_FORMAT_RAW_Gr  SENSOR_OUTPUT_FORMAT_RAW_R  SENSOR_OUTPUT_FORMAT_RAW_B
	.mclk = 24,
...
static kal_uint32 return_sensor_id(void)
{
    ...
    pr_debug("fsd----4h7 return_sensor_id\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			pr_debug("lpp----4h7 sensor id is 0x%x  i2c write id: 0x%x,\n",*sensor_id,imgsensor.i2c_write_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			pr_debug("s5k4h7mipiraw open sensor_id = %x\r\n", sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
...
/* enable = false; */
	if (enable) {
		/* 0x0601[2:0]; 0 no pattern; 1 solid; 2 full color bar; 3 fade to gray color bar */
