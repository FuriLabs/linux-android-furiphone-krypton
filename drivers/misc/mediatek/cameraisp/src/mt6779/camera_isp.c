...
#define TSTP_V3						(0)
#define DUMP_3A_TIME_PERIOD			(0)
#define WAIT_HW_RESET				(0)
#define ALSK_TBD					(1)
...
static void ISP_DumpDmaDeepDbg(enum ISP_IRQ_TYPE_ENUM module)
{
	...
	IRQ_LOG_KEEPER(module, m_CurrentPPB, _LOG_ERR,
		"%s ModuleGroup[%d] Req/Rdy Status:Req/Rdy_0=(0x%08x 0x%08x), Req/Rdy_1=(0x%08x 0x%08x),Req/Rdy_2=(0x%08x 0x%08x), Req/Rdy_3=(0x%08x 0x%08x),Req/Rdy_4=(0x%08x 0x%08x), Req/Rdy_5=(0x%08x 0x%08x),Req/Rdy_6=(0x%08x 0x%08x)",
		...
		case ISP_RT_BUF_CTRL_CLEAR:
			...
			case ISP_IRQ_TYPE_INT_CAM_C_ST:
				if ((pstRTBuf[rt_buf_ctrl.module]
					...
					g_ISPIntStatus[
					rt_buf_ctrl.module].ispInt4Err = 0;
	...
	enum ISP_IRQ_TYPE_ENUM module)
{
	...
	if (ErrStatus) {
		...
		case ISP_IRQ_TYPE_INT_CAM_A_ST:
			...
			g_ISPIntStatus[
			ISP_IRQ_TYPE_INT_CAM_A_ST].ispInt4Err |=
				WarnStatus;
		...
		case ISP_IRQ_TYPE_INT_CAM_B_ST:
			...
			g_ISPIntStatus[
			ISP_IRQ_TYPE_INT_CAM_B_ST].ispInt4Err |=
				(WarnStatus);
		...
		case ISP_IRQ_TYPE_INT_CAM_C_ST:
			...
			g_ISPIntStatus_SMI[
			ISP_IRQ_TYPE_INT_CAM_C_ST].ispInt4Err =
				g_ISPIntStatus[
				ISP_IRQ_TYPE_INT_CAM_C_ST].ispInt4Err;
		...
		case ISP_IRQ_TYPE_INT_CAMSV_6_ST:
			g_ISPIntStatus[
			ISP_IRQ_TYPE_INT_CAMSV_6_ST].ispIntErr |=
				(ErrStatus|WarnStatus);
		...
		case ISP_IRQ_TYPE_INT_CAMSV_7_ST:
			g_ISPIntStatus[
			ISP_IRQ_TYPE_INT_CAMSV_7_ST].ispIntErr |=
				(ErrStatus|WarnStatus);
	...
	unsigned int delayCheck)
{
	...
	if (bQueMode) {
		for (i = 0; i < _cam_max_; i++) {
			...
			if (fbc_ctrl1[dma_arry_map[i]].Raw != 0) {
#if TSTP_V3 /* (TIMESTAMP_QUEUE_EN == 1) */
				if (delayCheck == 0) {
					if (fbc_ctrl2[
...
static int32_t ISP_WaitTimestampReady(unsigned int module, unsigned int dma_id)
{
	...
	LOG_INF("Wait module:%d dma:%d timestamp ready W/R:%d/%d\n",
		...
		(unsigned int)IspInfo.TstpQInfo[module].Dmao[
		dma_id].TotalRdCnt);
