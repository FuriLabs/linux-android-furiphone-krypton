...
#define DTS_OF_NAME	"gcore,touchscreen"
...
{
	int ret;//prize-Solve TP glove mode problem-pengzhipeng-20220805
	if (ili_sleep_handler(TP_RESUME) < 0)
		ILI_ERR("TP resume failed\n");
//prize-Solve TP glove mode problem-pengzhipeng-20220805-start	
	if (ilits->glove_mode) 
	{
		printk("tpd_resume [Mode]exit glove mode");
		ret = ili_ic_func_ctrl("glove", 0x01);
		if (ret < 0) {
			printk("tpd_resume Glove Off fail");
		}
    }
//prize-Solve TP glove mode problem-pengzhipeng-20220805-end
}
...
static int __init ilitek_plat_dev_init(void)
{
	...
	if (lcm_name) {
		...
		if (!videolfb_tag)
			ILI_INFO("pzp read lcm name : %s\n", videolfb_tag->lcmname);
	}
	ILI_INFO("pzp ead lcm name : %s %d\n", videolfb_tag->lcmname, strcmp(videolfb_tag->lcmname, "ili7807s_fhdp_dsi_vdo_auo_120hz_drv") );

	
	if (strcmp(videolfb_tag->lcmname, "ili7807s_fhdp_dsi_vdo_auo_120hz_drv") != 0)
	{
		ILI_INFO("pzp ILITEK TP driver init for MTK111\n");
...
module_init(ilitek_plat_dev_init);//prize-modify-pengzhipeng-20220708-start
