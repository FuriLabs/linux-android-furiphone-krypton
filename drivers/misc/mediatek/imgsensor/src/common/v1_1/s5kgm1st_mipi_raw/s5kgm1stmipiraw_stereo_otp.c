...
#define PFX "S5KGM1ST_STEREO_OTP"
#undef LOG_INF
...
int write_dualcam_cali_flag_file_s5kgm1st(void)
{
...
int write_dualcam_cali_result_flag_file_s5kgm1st(int result)
{
...
#if 0
int dump_dualcam_cali_flag_file_s5kgm1st(void)
{
    struct file *eeprom_file = NULL;
    int err = 0;
    mm_segment_t fs = get_fs();
...
int dump_dualcam_cali_result_flag_file_s5kgm1st(int result)
{
    struct file *result_file = NULL;
    int err = 0;
    mm_segment_t fs = get_fs();
...
int store_dualcam_cali_data_s5kgm1st(void)
{
    int size = -1, rc = -1;
    BYTE checkSum = 0;
	int ret = 1;
...
int dump_dualcam_cali_data_s5kgm1st(void)
{
    struct file *eeprom_file = NULL;
