...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
	    ...
	    mdelay(20); // prize add by zhuzhengjiang for Abnormal color  low probability
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		// 0x5E00[1:0]; 00 Color bar, 01 Random Data, 10 Square, 11 BLACK
         ...
         #endif
         write_cmos_sensor(0x020E, 0x0000);//Gr
         write_cmos_sensor(0x0210, 0x0000);//R
         write_cmos_sensor(0x0212, 0x0000);//B
         write_cmos_sensor(0x0214, 0x0000);//Gb
         write_cmos_sensor(0x0204, 0x0000);// a-gain 

         write_cmos_sensor(0x0600, 0x0001);//02
			...
			switch (*feature_data) {
				case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 1;
					break;
				case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0; // video & capture use same setting  ////1
					break;
				case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
