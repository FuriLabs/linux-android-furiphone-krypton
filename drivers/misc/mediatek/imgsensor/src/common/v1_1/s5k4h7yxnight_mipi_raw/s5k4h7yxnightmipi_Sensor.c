...
#include "s5k4h7yxnightmipi_Sensor.h"


#define PFX "s5k4h7yxnight_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K4H7YXNIGHT_SENSOR_ID,

	.checksum_value = 0xd0fe109e, // /*prize  add for ATA checksum by zhuzhengjiang 20190307 start*/
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gr, //SENSOR_OUTPUT_FORMAT_RAW_MONO,  // prize add by zhuzhengjiang  for mono 20210727 
	.mclk = 24,
	.mipi_lane_num = SENSOR_MIPI_4_LANE,
	//prize fengshangdong modify at 20181030
...
static kal_uint16 read_cmos_nightsensor(kal_uint32 addr)
{
...
unsigned char S5K4H7_read_cmos_nightsensor(u32 addr)
{
	kal_uint16 get_byte = 0;
...
void S5K4H7_write_cmos_nightsensor(u16 addr, u32 para)
{
    char pusendcmd[3] = {(char)(addr >> 8), (char)(addr & 0xFF), (char)(para & 0xFF)};
...
static void preview_setting(void)
{
	...
	for (i = 0; i < 100; i++)
	{
	     framecnt = read_cmos_nightsensor(0x0005); /* waiting for sensor to  stop output  then  set the  setting */
	     if (framecnt == 0xFF)
	     {
...
static void capture_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor_8(0X0100, 0X00);
	 ...
	 for (i = 0; i < 100; i++)
	{
	    framecnt = read_cmos_nightsensor(0x0005);
	     if (framecnt == 0xFF)
	     {
		 LOG_INF("stream is  off\\n");
		  break;
							 ...
							 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ_BY_SCENARIO:
		...
		case SENSOR_FEATURE_GET_REGISTER:
			sensor_reg_data->RegData = read_cmos_nightsensor(sensor_reg_data->RegAddr);
			break;
