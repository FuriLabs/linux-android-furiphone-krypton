...
static uint8_t fm78100_chip_id(void)
{
	uint8_t chip_id =0;
...
static void fm78100_chip_init(void)
{
	s32 chip_id =0;
	int16_t  i =0 ;	
	int ret;
...
static void fm78100_chip_disable(void)
{
	exergen_i2c_write(0x09,0x03);//disable pd
	if(exergen_pdn_pin > 0)
	{
	gpio_direction_output(exergen_pdn_pin,0);
	}
}

static void fm78100_chip_enable(void)
{
	if(exergen_pdn_pin > 0)
	{
...
static void fm78100_get_temp(void)
{
	return;
}
static int32_t fm78100_read_ch1(void)
{
	uint8_t databuf[3];
	int32_t data_temp;
...
static int32_t fm78100_read_ch2(void)
{
	uint8_t databuf[3];
	int32_t data_temp;
...
{
	EXERGEN_DMESG("[fm78100]: Add FM78100 exergen driver!!");

	
    if (exergen_driver_add(&fm78100_device_driver) < 0) {
        EXERGEN_DMESG("[fm78100]: Add FM78100 exergen driver failed!!");
    }
    return 0;
}
