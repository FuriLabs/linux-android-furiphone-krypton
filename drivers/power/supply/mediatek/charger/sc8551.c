...
struct rt9759_chip *adc_enablchip;
...
static const struct rt9759_dtprop rt9759_dtprops_bool[] = {
	...
	RT9759_DT_VALPROP(adc_dis, RT9759_REG_ADCCTRL, 7, 0x80, NULL, 0),
...
int set_sc8551_adc_disenable(int enable){
  int ret;
if(adc_enablchip!=NULL){  
		 if(1==enable){
		    printk("lpp---plug in usb charg open 8551 adc\n");
		   ret=rt9759_set_bits(adc_enablchip,RT9759_REG_ADCCTRL,RT9759_ADCEN_MASK);
		 }else{
		 printk("lpp---plug out usb charg close 8551 adc\n");
 
		 ret=rt9759_clr_bits(adc_enablchip,RT9759_REG_ADCCTRL,RT9759_ADCEN_MASK);
 
		 }
 }else{
   printk("lpp---set sc8551 adc error\n");
 }
