...
{
	pr_debug("weak function of %s: fsel=%d, dbc=%d\n",
					__func__, fsel, dbc);
}

int __attribute__((weak)) dcm_smc_get_cnt(int type_id)
{
	pr_debug("weak function of %s: dcm->type_id=%d\n",
					__func__, type_id);
...
{
	pr_debug("weak function of %s: msg=%d\n", __func__, msg);
}
...
{
	pr_debug("weak function of %s: cci=%u\n", __func__, cci);
...
{
	pr_debug("weak function of %s: mp0=%u\n", __func__, mp0);
...
{
	pr_debug("weak function of %s: mp1=%u\n", __func__, mp1);
...
{
	pr_debug("weak function of %s: mp2=%u\n", __func__, mp2);
...
void dcm_set_default(unsigned int type)
{
	...
	pr_debug("[%s]type:0x%08x, init_dcm_type=0x%x\n",
					__func__, type, init_dcm_type);
#else
	pr_debug("[%s]type:0x%08x, init_dcm_type=0x%x, INIT_DCM_TYPE_BY_K=0x%x\n",
...
void dcm_set_state(unsigned int type, int state)
{
	...
	pr_debug("[%s]type:0x%08x, set:%d, init_dcm_type_pre=0x%x\n",
	...
	if (init_dcm_type_pre != init_dcm_type) {
		pr_debug("[%s]type:0x%08x, set:%d, init_dcm_type=0x%x->0x%x\n",
			__func__, type, state,
			init_dcm_type_pre,
			init_dcm_type);
		dcm_smc_msg_send(init_dcm_type);
...
void dcm_disable(unsigned int type)
{
	...
	if (init_dcm_type_pre != init_dcm_type) {
		pr_debug("[%s]type:0x%08x, init_dcm_type=0x%x->0x%x\n",
...
void dcm_sync_hw_state(void)
{
	...
	for (i = 0, dcm = &dcm_array[0]; i < NR_DCM_TYPE; i++, dcm++) {
		if (dcm->func_is_on != NULL) {
			...
			pr_debug("[%-16s 0x%08x] sync hw state:%d (%d)\n",
				 dcm->name, dcm->typeid, dcm->current_state,
				 dcm->disable_refcnt);
		}
	}
		...
		if (!strcmp(cmd, "restore")) {
			...
			if (sscanf(buf, "%15s %x %d", cmd, &mask, &mode) == 3) {
				...
				if (mask & STALL_DCM_TYPE) {
					if (mode)
						pr_debug("stall dcm is enabled for Default(Normal) mode started\n");
					else
						pr_debug("stall dcm is disabled for Performance(Sports) mode started\n");
				}
			}
		} else {
			pr_debug("SORRY, do not support your command: %s\n",
				    cmd);
		}
		ret = n;
	} else {
		pr_debug("SORRY, do not support your command.\n");
		ret = -EINVAL;
	}
