...
static int sgm41512_charge_type_detect_second(struct sgm41512_chip *chip)
{
	...
	bool hiz_state = false;

	ret = __sgm41512_is_hz_enabled(chip, &hiz_state);
	if (hiz_state){
...
static int sgm41512_set_pep_current_pattern(struct charger_device *chg_dev,
	bool is_increase)
{
	int ret = 0;
	struct sgm41512_chip *chip = dev_get_drvdata(&chg_dev->dev);
	...
	/* Set Pump Up/Down */
	if(is_increase == true){
		__sgm41512_set_ichg(chip, 1500000);
...
static int sgm41512_set_pep20_reset(struct charger_device *chg_dev)
{
	int ret = 0;
	struct sgm41512_chip *chip = dev_get_drvdata(&chg_dev->dev);

	ret = __sgm41512_set_aicr(chip, 100000);
	if (ret < 0)
		goto out;

	msleep(250);

	ret = __sgm41512_set_aicr(chip, 700000);
...
static struct charger_ops sgm41512_chg_ops = {
	...
	/* PE+/PE+20 */
	.send_ta_current_pattern = sgm41512_set_pep_current_pattern,
	.reset_ta = sgm41512_set_pep20_reset,
};
#if 1
