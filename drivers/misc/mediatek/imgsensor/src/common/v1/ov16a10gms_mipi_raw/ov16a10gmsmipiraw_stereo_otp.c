...
#define PFX "OV16A10GMS_STEREO_OTP"
#undef LOG_INF
...
#define CALI_DATA_OFFESET     0x1500 // 0xD7A
#define CALI_DATA_FLAG_OFFSET 0x1D01 //0x157B
#define CALI_DATA_CHECKSUM_OFFSET 0x1D00  //0x157A
...
int write_dualcam_cali_flag_file_ov16a10(void)
{
...
int write_dualcam_cali_result_flag_file_ov16a10(int result)
{
...
#if 0
int dump_dualcam_cali_flag_file_ov16a10(void)
{
    struct file *eeprom_file = NULL;
    int err = 0;
    mm_segment_t fs = get_fs();
...
int dump_dualcam_cali_result_flag_file_ov16a10(int result)
{
    struct file *result_file = NULL;
    int err = 0;
    mm_segment_t fs = get_fs();
    ...
    pr_debug("dump_result_flag:%d to flag dir, %s \n", result, __func__);
...
int store_dualcam_cali_data_ov16a10(void)
{
    int size = -1, rc = -1;
    BYTE checkSum = 0;
	int ret = 1;
    ...
    rc = write_dualcam_cali_flag_file_ov16a10();
    if (rc < 0) {
    ...
    rc = write_dualcam_cali_result_flag_file_ov16a10(ret);
    if (rc < 0) {
...
int dump_dualcam_cali_data_ov16a10(void)
{
    struct file *eeprom_file = NULL;
    ...
    if (IS_ERR(eeprom_file)) {
        pr_debug("open epprom file error, /mnt/vendor/ArcSoftCali/ dir need %s", __func__);
        ret2 = 0;
		return ret2;
    }
...
#if 0
    err = dump_dualcam_cali_flag_file_ov16a10();
    ...
    err = dump_dualcam_cali_result_flag_file_ov16a10(ret2);
    if (err < 0) {
        pr_debug("%s: failed to write_dualcam_cali_flag_file\n", __func__);
        //return -1;
        ret2 = -1;
