...
//prize-Solve the problem of sudden change of power when the electricity meter is 0% turned off and turned on-20210908-pengzhipeng-start
...
#define BATTERY_UP_STEP_CHANGE    (90*queue_delayed_work_time)   //12min
#define BATTERY_DOWN_STEP_CHANGE  (15*queue_delayed_work_time)    //2min
...
EXPORT_SYMBOL(g_cw2017_capacity);
int g_cw2017_vol = 0;
EXPORT_SYMBOL(g_cw2017_vol);
int cw2017_exit_flag=0;
EXPORT_SYMBOL(cw2017_exit_flag);
int g_cw2017_bat_temperature_val = 0;
EXPORT_SYMBOL(g_cw2017_bat_temperature_val);
...
//prize-Solve the problem of sudden change of power when the electricity meter is 0% turned off and turned on-20210908-pengzhipeng-start
	...
	if ((cw_bat->charger_mode > 0) && (UI_SOC >= 95) && (UI_SOC <= cw_bat->capacity)) {
		cw_printk(KERN_INFO "up step charging_cnt=%d\n",charging_cnt);
		charging_cnt++;	
		if (charging_cnt > (BATTERY_UP_STEP_CHANGE / queue_delayed_work_time) ){
			UI_SOC = (cw_bat->capacity + 1) <= 100 ? (cw_bat->capacity + 1) : 100; 
			charging_cnt = 0;
			jump_flag = 1;
		}else{
	...
	if ((cw_bat->charger_mode == 0) && (UI_SOC <= cw_bat->capacity ) && (UI_SOC >= 90) && (jump_flag == 1)) {
		cw_printk(KERN_INFO "down step discharging_cnt=%d\n",discharging_cnt);

		discharging_cnt++;
		if (discharging_cnt > (BATTERY_DOWN_STEP_CHANGE / queue_delayed_work_time) ){
			if (UI_SOC >= cw_bat->capacity - 1){
				jump_flag = 0;
			} else {
...
//prize-Solve the problem of sudden change of power when the electricity meter is 0% turned off and turned on-20210908-pengzhipeng-start
	...
	if ((UNINIT_UDATA == udata) || (udata > 100))
	{
