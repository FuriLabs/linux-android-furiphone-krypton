 ...
 *============================================================================
 ****************************************************************************/
#define PFX "S5K2P7GMS_camera_sensor"
...
#include "s5k2p7gmsmipiraw_Sensor.h"
...
{
	pr_debug("s5k2p7gms:dummyline = %d, dummypixels = %d\n",
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	pr_debug("s5k2p7gms:framerate = %d, min framelength should enable %d\n",
...
static void check_streamoff(void)
{
	...
	pr_debug("s5k2p7gms:%s exit!\n", __func__);
}

static kal_uint32 streaming_control(kal_bool enable)
{
	pr_debug("s5k2p7gms:streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
...
static void write_shutter(kal_uint16 shutter)
{
	...
	if (imgsensor.autoflicker_en) {
	   ...
	   imgsensor.pclk / imgsensor.line_length * 10 / imgsensor.frame_length;
		...
		/* Extend frame length*/
		write_cmos_sensor(0x0340, imgsensor.frame_length);
		pr_debug("s5k2p7gms:(else)imgsensor.frame_length = %d\n",
	...
	pr_debug("s5k2p7gms:shutter =%d, framelength =%d\n",
...
/* */
	...
	pr_debug("s5k2p7gms:Add for N3D! shutterlzl =%d, framelength =%d\n",
		shutter, imgsensor.frame_length);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain < BASEGAIN || gain > 32 * BASEGAIN) {
		pr_debug("s5k2p7gms:Error gain setting");
	...
	spin_unlock(&imgsensor_drv_lock);
	pr_debug("s5k2p7gms:gain = %d , reg_gain = 0x%x\n", gain, reg_gain);
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	pr_debug("s5k2p7gms:image_mirror = %d\n", image_mirror);
...
kal_uint16 addr_data_pair_init1_s5k2p7gms[] = {
...
kal_uint16 addr_data_pair_init2_s5k2p7gms[] = {
	0x6028, 0x4000,
	0x3190, 0x0008,
	0x3246, 0x0001,
	0x3588, 0x0069,
...
{
	pr_debug("s5k2p7gms:%s() E\n", __func__);
	write_cmos_sensor(0x6028, 0x4000);
	write_cmos_sensor(0x0100, 0x0000);
	mdelay(3);         /* Wait value must be at least 20000 MCLKs*/

	table_write_cmos_sensor(addr_data_pair_init1_s5k2p7gms,
		sizeof(addr_data_pair_init1_s5k2p7gms) / sizeof(kal_uint16));

#if USE_TNP_BURST
	write_cmos_sensor(0x602A, 0x3E10);
	write_cmos_sensor(0x6004, 0x0001);

	pr_debug("s5k2p7gms:Using Burst Mode for TNP (%d)\n", (int)sizeof(uTnpArrayA_gms));

	iWriteRegI2C(
	    (u8 *)uTnpArrayA_gms, (u16)sizeof(uTnpArrayA_gms), imgsensor.i2c_write_id);
	...
	pr_debug("s5k2p7gms:Using Burst Mode for TNP (%d)\n", (int)sizeof(uTnpArrayB_gms));

	iWriteRegI2C(
	    (u8 *)uTnpArrayB_gms, (u16)sizeof(uTnpArrayB_gms), imgsensor.i2c_write_id);
...
#endif
	table_write_cmos_sensor(addr_data_pair_init2_s5k2p7gms,
		sizeof(addr_data_pair_init2_s5k2p7gms) / sizeof(kal_uint16));

}	/*	sensor_init  */

kal_uint16 addr_data_pair_preview_s5k2p7gms[] = {
	0x317A, 0x0000,
	0x317C, 0x0130,
	0x0340, 0x0E1A,
	0x0342, 0x1400,
...
static void preview_setting(void)
{
	...
	table_write_cmos_sensor(addr_data_pair_preview_s5k2p7gms,
		sizeof(addr_data_pair_preview_s5k2p7gms) / sizeof(kal_uint16));
...
kal_uint16 addr_data_pair_cap30fps_s5k2p7gms[] = {
	0x317A, 0x0100,
	0x317C, 0x0130,
	0x0340, 0x0E1A,
	0x0342, 0x1400,
...
kal_uint16 addr_data_pair_cap24fps_s5k2p7gms[] = {
	0x317A, 0x0100,
	0x317C, 0x0130,
	0x0340, 0x0E1A,
	0x0342, 0x1910,
...
kal_uint16 addr_data_pair_cap15fps_s5k2p7gms[] = {
	0x317A, 0x0100,
	0x317C, 0x0130,
	0x0340, 0x0E1A,
	0x0342, 0x2800,
...
{
	pr_debug("s5k2p7gms:%s() E! currefps:%d\n", __func__, currefps);

	/* full size 29.76fps*/
	/* capture setting 4640*3488  24MHz 560MHz 1464Mbps/lane*/
	if (currefps == 300) {
		table_write_cmos_sensor(addr_data_pair_cap30fps_s5k2p7gms,
					sizeof(addr_data_pair_cap30fps_s5k2p7gms) /
					sizeof(kal_uint16));
	} else if (currefps == 240) {
		table_write_cmos_sensor(addr_data_pair_cap24fps_s5k2p7gms,
					sizeof(addr_data_pair_cap24fps_s5k2p7gms) /
					sizeof(kal_uint16));
	} else { /*15fps*/
		table_write_cmos_sensor(addr_data_pair_cap15fps_s5k2p7gms,
					sizeof(addr_data_pair_cap15fps_s5k2p7gms) /
					sizeof(kal_uint16));
	}
	write_cmos_sensor(0x6214, 0x7970);
	if (imgsensor.pdaf_mode == 1) {/* enable PDAF Tail Mode */
...
kal_uint16 addr_data_pair_normal_video_s5k2p7gms[] = {
	0x317A, 0x0100,
	0x317C, 0x0130,
	0x0340, 0x0E1A,
	0x0342, 0x1400,
...
static void normal_video_setting(kal_uint16 currefps)
{
	pr_debug("s5k2p7gms:%s() E! currefps:%d\n", __func__, currefps);
	/* full size 30fps*/
	table_write_cmos_sensor(addr_data_pair_normal_video_s5k2p7gms,
	    sizeof(addr_data_pair_normal_video_s5k2p7gms) / sizeof(kal_uint16));

	write_cmos_sensor_8(0x0B0E, 0x01);

}

kal_uint16 addr_data_pair_hs_video_s5k2p7gms[] = {
	0x317A, 0x0000,
	0x317C, 0x0130,
	0x0340, 0x0388,
	0x0342, 0x1400,
...
static void hs_video_setting(void)
{
	pr_debug("s5k2p7gms:%s() E\n", __func__);
	/*720p 120fps*/
	table_write_cmos_sensor(addr_data_pair_hs_video_s5k2p7gms,
		sizeof(addr_data_pair_hs_video_s5k2p7gms) / sizeof(kal_uint16));

	write_cmos_sensor_8(0x0B0E, 0x01);

}

kal_uint16 addr_data_pair_slim_video_s5k2p7gms[] = {
	0x317A, 0x0000,
	0x317C, 0x0130,
	0x0340, 0x0E1A,
	0x0342, 0x1400,
...
static void slim_video_setting(void)
{
	pr_debug("s5k2p7gms:%s() E\n", __func__);
	/*1080p 60fps*/
	table_write_cmos_sensor(addr_data_pair_slim_video_s5k2p7gms,
		sizeof(addr_data_pair_slim_video_s5k2p7gms) / sizeof(kal_uint16));
 ...
 *
 *************************************************************************/
 #define S5K2P7GMSGMS_MODULE_ID_OFFSET  0x0003
static kal_uint16 read_s5k2p7gmsgmsmodule_id(void)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = {(char)(S5K2P7GMSGMS_MODULE_ID_OFFSET >> 8), (char)(S5K2P7GMSGMS_MODULE_ID_OFFSET & 0xFF)};

	iReadRegI2C(pusendcmd, 2, (u8 *)&get_byte, 1, 0xA0/*EEPROM_READ_ID*/);   //A0 && B0 ????
	pr_err("s5k2p7gms_read_ylmodule_id id is %d\n", get_byte);
	return get_byte;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	pr_debug("[5k2p7_camera_sensor] imgsensor_info.sensor_id:%x\n",imgsensor_info.sensor_id);
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		pr_debug("[5k2p7_camera_sensor] imgsensor_info.i2c_addr_table[i]:%x\n",imgsensor_info.i2c_addr_table[i]);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = (
	      (read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001)) +2;

			pr_debug("[5k2p7_camera_sensor] get_imgsensor_id :%x\n",*sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {

				pr_debug("s5k2p7gms:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);

				return ERROR_NONE;
			}
			pr_debug("s5k2p7gms:Read sensor id fail, id: 0x%x\n",
...
static kal_uint32 get_info(enum MSDK_SCENARIO_ID_ENUM scenario_id,
				  ...
				  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_debug("s5k2p7gms:scenario_id = %d\n", scenario_id);
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			break;
	default:
			pr_debug("s5k2p7gms:Error ScenarioId setting");
			preview(image_window, sensor_config_data);
...
{
	pr_debug("s5k2p7gms:framerate = %d\n ", framerate);
...
{
	pr_debug("s5k2p7gms:enable = %d, framerate = %d\n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
	...
	enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	pr_debug("s5k2p7gms:scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	struct SET_SENSOR_PATTERN_SOLID_COLOR *pdata)
{
	...
	pr_debug("s5k2p7gms:set_test_pattern enum: %d\n", modes);
	if (modes) {
		write_cmos_sensor(0x0600, modes);
		if (modes == 1 && (pdata != NULL)) { //Solid Color
			pr_debug("s5k2p7gms:R=0x%x,Gr=0x%x,B=0x%x,Gb=0x%x",
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	pr_debug("s5k2p7gms:feature_id = %d\n", feature_id);
	switch (feature_id) {
	...
	case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
		pr_debug("s5k2p7gms:imgsensor.pclk = %d,current_fps = %d\n",
	...
	case SENSOR_FEATURE_SET_TEST_PATTERN:
		...
		pr_debug("s5k2p7gms:SENSOR_FEATURE_SET_TEST_PATTERN Enum:%d\n", (UINT32)*feature_data);
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		pr_debug("s5k2p7gms:current fps :%d\n", *feature_data_32);
		spin_lock(&imgsensor_drv_lock);
	...
	case SENSOR_FEATURE_SET_HDR:
		pr_debug("s5k2p7gms:hdr enable :%d\n", *feature_data_32);
		spin_lock(&imgsensor_drv_lock);
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		pr_debug("s5k2p7gms:SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n",
				(UINT32)*feature_data);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		pr_debug("s5k2p7gms:SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
			(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		pr_debug("s5k2p7gms:SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		pr_debug("s5k2p7gms:SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%d\n",
			(UINT16)*feature_data);
		PDAFinfo =
	...
	case SENSOR_FEATURE_GET_VC_INFO:
		pr_debug("s5k2p7gms:SENSOR_FEATURE_GET_VC_INFO %d\n",
			(UINT16)*feature_data);
		pvcinfo =
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:	/*get cal data from eeprom*/
			pr_debug("s5k2p7gms:SENSOR_FEATURE_GET_PDAF_DATA\n");
			read_2P7GMS_eeprom((kal_uint16)(*feature_data),
				(char *)(uintptr_t)(*(feature_data+1)),
					(kal_uint32)(*(feature_data+2)));
			pr_debug("s5k2p7gms:SENSOR_FEATURE_GET_PDAF_DATA success\n");
			break;
	case SENSOR_FEATURE_SET_PDAF:
			pr_debug("s5k2p7gms:PDAF mode :%d\n", *feature_data_16);
	...
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		pr_debug("s5k2p7gms:SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		pr_debug("s5k2p7gms:SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(*feature_data);
...
UINT32 S5K2P7GMS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
