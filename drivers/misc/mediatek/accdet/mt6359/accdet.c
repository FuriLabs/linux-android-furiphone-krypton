	...
	const char *buf, size_t count)
{
	...
	if (ret) {
		...
		if (IS_ERR(thread)) {
			...
			pr_debug("%s() create thread failed,err:%d\n",
				__func__, error);
		} else
	...
	const char *buf, size_t count)
{
	...
	if (ret < 0) {
		pr_debug("%s() kstrtoint failed! ret:%d\n", __func__, ret);
		return ret;
	}
...
static int accdet_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		...
		if (err) {
			pr_debug("%s() driver_create_file %s err:%d\n",
...
static bool accdet_timeout_ns(u64 start_time_ns, u64 timeout_time_ns)
{
	...
	if (cur_time < start_time_ns) {
		pr_debug("%s Timer overflow! start%lld cur timer%lld\n",
			__func__, start_time_ns, cur_time);
		start_time_ns = cur_time;
		/* 400us */
		timeout_time_ns = 400 * 1000;
		pr_debug("%s Reset timer! start%lld setting%lld\n",
...
static void send_accdet_status_event(u32 cable_type, u32 status)
{
	...
	case HEADSET_NO_MIC:
		...
		input_sync(accdet_input_dev);
		pr_debug("%s HEADPHONE(3-poole) %s\n", __func__,
...
extern int mtk_nanohub_calibration_to_hub(uint8_t sensor_id);
...
//prize-ACCDET sar calibration-pengzhipeng-2022812-start 
		mtk_nanohub_calibration_to_hub(71);	
//prize-ACCDET sar calibration-pengzhipeng-2022812-end 
//prize added by huarui, headset support, 20190111-end
	} else {
...
static inline void check_cable_type(void)
{
	...
	cur_AB = pmic_read(PMIC_ACCDET_MEM_IN_ADDR) >> ACCDET_STATE_MEM_IN_OFFSET;
	cur_AB = cur_AB & ACCDET_STATE_AB_MASK;
	pr_debug("accdet %s(), cur_status:%s current AB = %d\n", __func__,
...
static inline int ext_eint_setup(struct platform_device *platform_device)
{
	...
	if (!node) {
		pr_debug("accdet %s can't find compatible node\n", __func__);
		return -1;
	}
	...
	if (ret < 0) {
		pr_debug("accdet %s gpiodebounce not found,ret:%d\n",
			__func__, ret);
		return ret;
	}
	gpio_set_debounce(gpiopin, gpio_headset_deb);
	...
	ret = of_property_read_u32_array(node, "interrupts", ints,
			ARRAY_SIZE(ints));
	if (ret) {
		pr_debug("accdet %s interrupts not found,ret:%d\n",
			__func__, ret);
		return ret;
	}
	...
	ret = request_irq(accdet_irq, ex_eint_handler, IRQF_TRIGGER_NONE,
		"accdet-eint", NULL);
	if (ret) {
		pr_debug("accdet %s request_irq fail, ret:%d.\n", __func__,
			ret);
		return ret;
	}
...
static int accdet_get_dts_data(void)
{
	...
	node = of_find_matching_node(node, accdet_of_match);
	if (!node) {
		pr_debug("%s can't find compatible dts node\n", __func__);
		return -1;
	}
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	if (ret) {
		if (ret != -EPROBE_DEFER)
			pr_debug("%s(), Error: Get iio ch failed (%d)\n", __func__, ret);
		return -EPROBE_DEFER;
	}
#endif
	...
	if (ret) {
		pr_debug("%s alloc_chrdev_reg fail,ret:%d!\n", __func__, ret);
	...
	if (ret) {
		pr_debug("%s cdev_add fail.ret:%d\n", __func__, ret);
		goto err_cdev_add;
	}
	...
	if (ret) {
		pr_debug("%s input_register_device fail.ret:%d\n", __func__,
			ret);
		goto err_input_reg;
	}
	...
	if (ret) {
		pr_debug("%s create_attr fail, ret = %d\n", __func__, ret);
	...
	if (!accdet_workqueue) {
		ret = -1;
		pr_debug("%s create accdet workqueue fail.\n", __func__);
		goto err_create_attr;
	}
	...
	if (ret) {
		...
		pr_debug("%s accdet_get_dts_data err!\n", __func__);
		goto err;
	}
	...
	if (!dis_micbias_workqueue) {
		ret = -1;
		pr_debug("%s create dis micbias workqueue fail.\n", __func__);
		goto err;
	}
	...
	if (!eint_workqueue) {
		ret = -1;
		pr_debug("%s create eint workqueue fail.\n", __func__);
	...
	if (ret) {
		pr_debug("%s ap eint setup fail.ret:%d\n", __func__, ret);
		goto err_eint_setup;
	}
#endif
