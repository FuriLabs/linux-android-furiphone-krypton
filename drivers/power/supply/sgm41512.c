...
/* prize hanjiuping modified 20210916 for ETA6953 start */
/* It seems we would get wrong data with block read sometimes,
 * so implement block read/write with multi-single method
 */
					  ...
					  u32 len, u8 *data)
{
	...
	for (; reg < cmd + len; reg++) {
#ifdef CONFIG_RT_REGMAP
		ret = rt_regmap_block_read(chip->rm_dev, reg, 1, &data[reg - cmd]);
#else
		ret = sgm41512_read_device(chip->client, reg, 1, &data[reg - cmd]);
#endif /* CONFIG_RT_REGMAP */
		if (ret < 0) {
			dev_err(chip->dev, "%s: read REG%02X(%02X...%02X) failed with %d\n",
				__func__, reg, cmd, cmd + len - 1, ret);
			break;
		}
	}
...
/* prize hanjiuping modified 20210916 for ETA6953 end */
					   ...
					   u32 len, const u8 *data)
{
	...
	for (; reg < cmd + len; reg++) {
#ifdef CONFIG_RT_REGMAP
		ret = rt_regmap_block_write(chip->rm_dev, reg, 1, &data[reg - cmd]);
#else
		ret = sgm41512_write_device(chip->client, reg, 1, &data[reg - cmd]);
#endif /* CONFIG_RT_REGMAP */
		if (ret < 0) {
			dev_err(chip->dev, "%s: write REG%02X(%02X...%02X) failed with %d\n",
				__func__, reg, cmd, cmd + len - 1, ret);
			break;
		}
	}
...
static irqreturn_t sgm41512_irq_handler(int irq, void *data)
{
	...
	/*
	dev_dbg(chip->dev, "%s: block_read: evt[0] = 0x%x, evt[1] = 0x%x, evt[2] = 0x%x\n",
		__func__, evt[0], evt[1], evt[2]);
	*/
