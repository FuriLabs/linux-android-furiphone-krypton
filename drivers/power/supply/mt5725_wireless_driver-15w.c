...
//prize larks add for compile error  start 
/*struct charger_data;
struct charger_data {
...
//prize larks add for compile error end 
...
struct otg_wireless_ctl {
	...
	struct pinctrl_state *charger_otg_off, *charger_otg_on,*wireless_5725_off,*wireless_5725_on,*charger_otg_mode_on,*charger_otg_mode_off;
...
static ssize_t enable_tx_store(struct device* dev, struct device_attribute* attr, const char* buf, size_t count)
{
    ...
    if(temp==1) {
			...
			turn_on_otg_charge_mode(1);  //GPIO109---->high
			pr_debug(KERN_INFO"mt5725 333 enable tx\n");
			turn_on_rever_5725(0);//OD5-->high
...
int get_mt5725_charge_protocol(void)
{
	return mte->charge_protocol;
}
EXPORT_SYMBOL(get_mt5725_charge_protocol);
...
//prize add by lipenpeng 20210419 start   EPP  agreement 9V
		    ...
		    /*set vout to 9000mv*/
		    set_rx_vout(9000);
			En_Dis_add_current(0x00);//prize add by lipengpeng 20210622 start 
#endif
	 ...
	 if(temp.ptr[0] == 0){
		...
		if(mte->wireless_max_power >=10){
			...
			if(ifit < tcur.value) {
				...
				pr_debug("[%s] En_Dis_add_current : DISABLE\n",__func__);
			}
		}
	 }else{
...
int turn_off_rever_5725(int en){
    ...
    if (mte->otg_5725_ctl.gpio_otg_prepare) {
		if (en) {
			pinctrl_select_state(mte->otg_5725_ctl.pinctrl_gpios, mte->otg_5725_ctl.wireless_5725_off);//GPOD5  low
			pr_debug("%s: set W_OTG_EN2 to hight\n", __func__);
			ret =0;
		}
		else {
			pinctrl_select_state(mte->otg_5725_ctl.pinctrl_gpios, mte->otg_5725_ctl.wireless_5725_on);//GPOD5  high
			pr_debug("%s: set W_OTG_EN2 to low\n", __func__);
			ret =0;
		}
	}
