	...
	unsigned int channel_num)
{
	...
	pr_debug("[debug][%s] %s: running,  channel_num = %u \r\n",
		LOG_FLAG, __func__,  channel_num);
	...
	for (i = 0; i < init_list->num; i++) {
		...
		if (0 != ret) {
			pr_warn("[ warn][%s] %s: ret = %d,  regmap = %p, addr = 0x%x\r\n",
				LOG_FLAG, __func__, ret, regmap, init_regs[i].addr);
			break;
		}
	}
}
	...
	const SIPA_PARAM_LIST *list, SIPA_REG_PROC *regs, uint32_t val_width)
{
	...
	for (i = 0; i < list->num; i++) {
		...
		if (0 != ret) {
			pr_err("[  err][%s] %s: ret = %d,  regmap = %p, addr = 0x%x\r\n",
				LOG_FLAG, __func__, ret, si_pa->regmap, regs[i].addr);
			return ret;
		}
	}
	...
	uint8_t *buff, uint32_t buff_len)
{
	...
	for (j = 0; j < list->num; j++) {
		...
		if (0 != ret) {
			pr_err("[  err][%s] %s: ret = %d,  regmap = %p, addr = 0x%x\r\n",
				LOG_FLAG, __func__, ret, si_pa->regmap, regs[j].addr);
			return 0;
		}
