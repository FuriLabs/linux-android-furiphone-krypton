 ...
 * DW9781C : OIS driver
 * AK7377  : VCM driver be the same as AK7371AF
 */
...
#define AK7377AF_I2C_SLAVE_ADDR 0x18
...
{
	LOG_INF("DW9781C Timeout Handle Flow\n");
...
int s4EEPROM_ReadReg_DW9781CAF(unsigned short addr, unsigned short *data)
{
	int i4RetValue = 0;
...
int s4AF_WriteReg_DW9781CAF(unsigned short i2c_id, unsigned char *a_pSendData,
...
int s4AF_ReadReg_DW9781CAF(unsigned short i2c_id, unsigned char *a_pSendData,
			   unsigned short a_sizeSendData,
...
static inline int setAFPara(__user struct stAF_MotorCmd *pstMotorCmd)
{
	...
	case 1:
		set_OIS_ENABLE(stMotorCmd.u4Param); /* 1 : disable */
		break;
	case 2:
		break;
	}
		     ...
		     unsigned long a_u4Param)
{
	...
	case AFIOC_G_MOTOROISINFO:
		...
		break;
	/*prize add by zhuzhengjiang for ois gyro offset calibration start*/
	case AFIOC_G_GETGRYOFFSETCALI:
		ois_reset();//Main_OIS();
		mdelay(100);
		dw9781c_download_ois_fw();
		mdelay(100);
		...
		break;
	case AFIOC_S_SETOISMODE:
		i4RetValue = set_OIS_Mode(a_u4Param);
		break;
	/*prize add by zhuzhengjiang for ois gyro offset calibration end*/
...
int DW9781CAF_PowerDown(struct i2c_client *pstAF_I2Cclient,
			int *pAF_Opened)
{
