...
#define IRQ_LOG_KEEPER(irq_in, ppb_in, logT_in, fmt, ...) do {\
	...
	if (avaLen > 1) {\
		if (snprintf((char *)(pDes), avaLen, "[%d.%06d]" fmt,\
			...
			##__VA_ARGS__) < 0) { \
			pr_debug("[Error] snprintf failed\n");\
		} \
...
#define IRQ_LOG_KEEPER_PR_ERR(irq_in, ppb_in, logT_in, fmt, ...) do {\
	...
	if (avaLen > 1) {\
		...
		if ('\0' != gSvLog[irq]._str[ppb][logT][str_leng - 1]) {\
			pr_debug("error:log str over flow(%d)", irq);\
		} \
		while (*ptr++ != '\0') {        \
			(*ptr2)++;\
		}     \
...
enum ISP_P2_BUFQUE_LIST_TAG listTag, signed int idxU)
{
	...
	if (idxU < 0) {
		pr_debug("idxU abnormal error(%d)\n", idxU);
		return ret;
	}
	idx = idxU;
	property = propertyU;
		...
		enum ISP_P2_BUFQUE_MATCH_TYPE type, signed int *idx)
{
	...
	case ISP_P2_BUFQUE_MATCH_TYPE_WAITDQ:
		index = *idx;
		if (index < 0) {
			pr_debug("index abnormal error(%d) 1\n", index);
			return ret;
		}
	...
	case ISP_P2_BUFQUE_MATCH_TYPE_WAITFM:
		index = *idx;
		if (index < 0) {
			pr_debug("index abnormal error(%d) 2\n", index);
			return ret;
		}
		spin_lock(&(SpinLock_P2FrameList));
		local_idx = index;
...
static void ISP_ion_init(void)
{
	...
	if (IspInfo.DebugMask & ISP_DBG_ION_CTRL)
		pr_debug("create ion client 0x%p\n", pIon_client);
}
...
static void ISP_ion_uninit(void)
{
	...
	if (IspInfo.DebugMask & ISP_DBG_ION_CTRL)
		pr_debug("destroy ion client 0x%p\n", pIon_client);
						...
						int fd)
{
	...
	if (IspInfo.DebugMask & ISP_DBG_ION_CTRL)
		pr_debug("[ion_import_hd] Hd(0x%p)\n", handle);
	return handle;
}
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case  ISP_SET_SEC_DAPC_REG:
		pr_debug("ISP_SET_SEC_DAPC_REG = %d\n", ISP_SET_SEC_DAPC_REG);
		if (copy_from_user(
		    Dapc_Reg,
		    (void *)Param,
	...
	struct file *pFile)
{
	...
	/*  */
	/* pr_debug("UserCount(%d)",IspInfo.UserCount); */
	/*  */
	if (pFile->private_data != NULL) {
		pUserInfo = (struct ISP_USER_INFO_STRUCT *)pFile->private_data;
...
pr_debug("CONFIG_MTK_CLKMGR get clock pointer \n");
...
static void SMI_INFO_DUMP(enum ISP_IRQ_TYPE_ENUM irq_module)
{
	...
	case ISP_IRQ_TYPE_INT_CAMSV_5_ST:
		...
		break;
	default:
		pr_debug("error:unsupported module:%d\n", irq_module);
		break;
	}
#endif
}
