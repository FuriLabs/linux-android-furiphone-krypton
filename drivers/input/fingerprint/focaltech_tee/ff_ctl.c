...
extern int ff_ctl_reset_device(uint32_t reset_level);
...
static void ff_ctl_device_event(struct work_struct *ws)
{
    ...
    FF_LOGV("%s(irq = %d, ..) toggled.", __func__, ctx->irq_num);
#ifdef CONFIG_PM_WAKELOCKS
/*prize modified by fae suggestion,jiffies_to_msecs(2*HZ)) change to 2000,The purpose of this modification is that the fingerprint code is not related to the platform,20220422,begin*/
    //__pm_wakeup_event(&g_context->wake_lock, jiffies_to_msecs(2*HZ));
    __pm_wakeup_event(&g_context->wake_lock, 2000);
/*prize modified by fae suggestion,jiffies_to_msecs(2*HZ)) change to 2000,The purpose of this modification is that the fingerprint code is not related to the platform,20220422,end*/
#else
...
static int ff_ctl_init_driver(void)
{
    ...
    do {
        ...
        if (err) {
	/*prize modified by fae suggestion,Avoid being compatible with multiple fingerprints to generate dump,If irq acquisition fails, release irq,20220422,begin*/
            //FF_LOGE("request_irq(..) = %d.", err);
            FF_LOGE("request_irq(..) = %d. set irq_num = %d, to -1", err, g_context->irq_num);
			g_context->irq_num = -1;
	/*prize modified by fae suggestion,Avoid being compatible with multiple fingerprints to generate dump,If irq acquisition fails, release irq,20220422,end*/
            break;
        }
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FF_IOC_RESET_DEVICE:{
        ...
        err = ff_ctl_reset_device(reset_level);
        break;
	}
...
static int __init ff_ctl_driver_init(void)
{
	...
	//id=mas_connect();
	//prize by ycj start
	sprintf(current_fingerprint_info.chip,"FT9391");
	//prize by ycj end
