...
{
	if (ucDownUp == 0) {
		WIFI_DBG_FUNC("Try to down wr_mtx\n");
		if (ucIsBlocking == 1)
			down(&wr_mtx);
	...
	if (copy_from_user(local, buf, copy_size) == 0) {
		...
		if (local[0] == '0') {
			...
			if (wlan_mode == WLAN_MODE_DUAL_P2P) {
				WIFI_DBG_FUNC("WIFI is already in dual p2p mode %d!\n", wlan_mode);
			} else {
				netdev = dev_get_by_name(&init_net, ifname);
				if (netdev && pf_set_p2p_mode) {
					p2pmode.u4Enable = 0;
			...
			if (wlan_mode == WLAN_MODE_DUAL_AP) {
				WIFI_DBG_FUNC("WIFI is already in dual ap mode %d!\n", wlan_mode);
			} else {
				netdev = dev_get_by_name(&init_net, ifname);
				if (netdev && pf_set_p2p_mode) {
					p2pmode.u4Enable = 0;
			...
			if (!strncmp(local + 9, "1", 1)) {
				WIFI_DBG_FUNC("local = %s, wifiSLog val = %s", local, local + 9);
...
static int WIFI_init(void)
{
	...
	if (fw_log_ics_init() < 0) {
		WIFI_DBG_FUNC("ics log node init failed!!\n");
		goto error;
	}
#endif
	return 0;
